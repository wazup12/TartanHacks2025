...
def update_edge_weights_with_fire(G, fire_mask, road_image_size=(800,800), fire_mask_size=(500,500), sigma=20, beta=1):
    ...
    Parameters:
      - G: networkx graph whose edges have an 'importance' attribute.
      - fire_mask: a 500x500 (grayscale) numpy array where white (255) indicates active fire.
      - road_image_size: size (width, height) used for the road/intersection images (e.g., (800,800)).
      - fire_mask_size: size (width, height) of the fire mask (e.g., (500,500)).
      - sigma: distance decay parameter (in fire mask pixels).
      - beta: multiplier controlling how strongly the fire influences the weight.
    
    The new weight is computed as:
        weight = importance * (1 + beta * exp(-distance/sigma))
    so that edges whose midpoint is very close to the fire (distance≈0) have increased weight.
    """
    ...
    for u, v, data in G.edges(data=True):
        ...
        # Get the base road importance (from your original mapping)
        road_importance = data["importance"]
        # Compute a new weight. (Adjust sigma and beta as needed.)
...
def generate_fire_heatmap(G, road_image_size=(800,800), line_thickness=2):
    ...
    Parameters:
      - G: networkx graph with updated 'weight' on each edge.
      - road_image_size: resolution of the output heatmap image.
      - line_thickness: thickness of the drawn edges.
    
    The heatmap is saved as 'fire_heatmap.png'.
    """
    # Create a blank grayscale image (note: image shape is (height, width))
    ...
    for u, v, data in G.edges(data=True):
        ...
        # Draw the line on the heatmap image. cv2.line will set the pixel value.
    ...
    cv2.imwrite("fire_heatmap.png", heatmap_color)
...
if __name__ == "__main__":
    ...
    intersections, bounds, road_image_size = create_street_and_intersection_maps(lat, lon, intersection_radius=3, dist=2000)
    
    if intersections is not None:
        # Fetch a road network at a (possibly different) scale to build the graph.
        edges, nodes, _ = fetch_street_network(lat, lon, dist=2000)
        ...
        # It is assumed that 'fire_mask.png' is a 500x500 grayscale image where white (255)
        # indicates areas of active fire.
        # ----------------------------------------------------------------------
        fire_mask = cv2.imread("mask_test.jpg", cv2.IMREAD_GRAYSCALE)
        if fire_mask is None:
            print("Fire mask image not found. Please provide a 500x500 image named 'fire_mask.png'.")
        else:
            ...
            # Generate and save the fire heatmap image.
