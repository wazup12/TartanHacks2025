...
# -------------------------------
def main():
    # (Commented out argparse section for simplicity)
    num_sat_points = 100
    center_lat = 40.4447605
    center_lon = -79.9426024
    zoom = 17 
    # wind_dir and wind_mag will be set randomly below.
    slope_dir = 0.0 
    grad_scale = 0.0025 
    steps = 250
    sigma = 35.0 
    opacity = 0.97 
    smoke_color = "30,30,30"
    ...
    for i in range(num_sat_points):
        ...
        for j, (burned_img, mask) in enumerate(zip(burned_images, fire_masks)):
            ...
            mask_filename   = os.path.join(labels_dir, f"{output_prefix}_sat_{i}_fire_mask_{j}.jpg")
            burned_img.save(burned_filename)
            
            # Convert the fire mask to an 8-bit grayscale image.
            # Multiply by 255 so that the "1" values become "255" (white).
            mask_img = Image.fromarray((mask * 255).astype(np.uint8))
            # Save with JPEG encoding.
            mask_img.save(mask_filename, format="JPEG")
