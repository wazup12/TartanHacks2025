#!/usr/bin/env python3
"""
Simulate fire spread over a satellite image and record new fire positions over time.
"""
...
def get_simulation_json():
    """
    Runs the entire simulation using default coordinates and returns the result as a JSON string.
    """
    # Default coordinates (San Francisco) and zoom level.
    lat, lon = 37.7749, -122.4194
    return get_simulation_json_for_coords(lat, lon)


# ------------------------------------------------------------------------------
# New function: Takes in a latitude and longitude and returns the simulation JSON.
# ------------------------------------------------------------------------------
def get_simulation_json_for_coords(lat, lon):
    ...
    # -------------------------------
    # Download satellite image for the provided coordinates.
    # -------------------------------
    zoom = 18
    ...
    depth_map = compute_depth_map_from_image(img, midas, transform, device)

    # -------------------------------
    # Run fire spread simulation (500 time steps).
    # -------------------------------
    ...
    # -------------------------------
    # Formula: resolution = cos(latitude) * 2π * EarthRadius / (256 * 2^zoom)
    ...
    # -------------------------------
    # Compute time series of new fire positions (every 10 steps).
    # -------------------------------
    time_series_positions = []
    for t in range(10, len(states), 10):
        prev_state = states[t - 10]
        curr_state = states[t]
        # New fires: cells that are on in the current state but were off 10 steps ago.
        ...
        for i, j in zip(new_indices[0], new_indices[1]):
            ...
            # In the grid, row index i increases downward.
...
def main():
    # Run the simulation using default coordinates and print the resulting JSON.
    simulation_json = get_simulation_json()
    print("Giant JSON output:")
